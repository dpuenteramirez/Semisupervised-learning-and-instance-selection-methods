...
Según~\cite{zhou2014semi} \textit{Semi-Supervised Learning} se define como una forma de entrenamiento de modelos el cual usa tanto datos etiquetados como no etiquetados, i.e. si no sería un aprendizaje supervisado, Sección~\ref{subsec:Aprendizaje-Supervisado}, o no supervisado, Sección~\ref{subsec:Aprendizaje-No-Supervisado}. 
...
%		\State Seleccionar las $p$ y $n$ instancias con mayor intervalo de confianza de cada clasificador
%		\State Añadirlas a $L$ y eliminarlas de $U'$
%		\State Elegir aleatoriamente $2p+2n$ instancias de $U$ y añadirlas a $U'$
%	\EndFor
%\EndProcedure
%\end{algorithmic}
%\end{algorithm}
...
\begin{algorithm}[H]
	\KwIn{Conjunto de entrenamiento $X = \lbrace\left( x_1, y_1 ,\right)\dots \left(x_n, y_n\right)\rbrace$, $k$ vecino.}
...
El algoritmo propuesto por Hart~\cite{hart1968condensed}, ver algoritmo~\ref{alg:Hart-CNN}, posee una complejidad computacional de $O(n^2)$. El conjunto obtenido a partir de $T$, i.e. $S$, operando con grandes conjuntos de datos demuestra poseer un tamaño considerablemente menor respecto a $T$. 
...
\begin{algorithm}[H]
  ...
  $S \leftarrow \text{Filtrado de ruido}(X)$ \\
	Ordenar las instancias de $S$ por la distancia a su enemigo más próximo \tcc*{de mayor a menor}
	\ForAll{$x \in S$}{
